"""
Phase 2 – Train the baseline Vision Transformer (linear patch embedding).
"""
...
from models.vit import ViTConfig, build_vit
...
def main():
    ...
    model = build_vit(vit_cfg).to(device)
    print(f"ViT parameters: {model.count_parameters():,}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics["accuracy"] > best_val_acc:
            ...
            save_checkpoint(
                ...
                str(weights_dir / "vit_best.pt"),
            )

    save_checkpoint(
        model, optimizer, epochs, val_metrics, str(weights_dir / "vit_last.pt")
    )

    plot_training_curves(
        history,
        save_path=str(figures_dir / "vit_training_curves.png"),
        title="Baseline ViT – Training Curves",
    )

    # Test evaluation
    ckpt = torch.load(weights_dir / "vit_best.pt", map_location=device)
    model.load_state_dict(ckpt["model_state_dict"])
    test_metrics = evaluate_model(
        model, loaders["test"], device, criterion, return_predictions=True
    )
    print_metrics(test_metrics, title="Baseline ViT – Test Results")
    plot_confusion_matrix(
        test_metrics["confusion_matrix"],
        class_names,
        save_path=str(figures_dir / "vit_confusion_matrix.png"),
        title="Baseline ViT – Confusion Matrix",
    )
